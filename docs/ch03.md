...
如果您是一家朝着这个方向发展的公司，则应该意识到，一旦代码库变成了[[spaghetti_code|意大利面条]]，几乎是不可能修复的。您可能会为产品的使用寿命付出高昂的开发成本。此外，好的（或坏的）设计的回报很快就会到来，因此战术方法很有可能甚至不会加快您的首个产品发布的速度。

> [!NOTE]
> 我个人反认为纯粹的面条要比没有想清楚的设计要好. 面条比较容易通过 ide 的功能实现逻辑的拆分, 而糟糕的设计就很难拆分.
> 但是更多的是面条和错误设计的混合.
