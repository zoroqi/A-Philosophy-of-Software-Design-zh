...
> [!NOTE]
> 现在不可能修改的(很难推动修改), 太多设备依赖了这个接口定义了. 但在不考虑任何历史包袱情况下 I/O 还会设计成这个样子吗? 我暂时没有想到的, 打开, 关闭, 读写和设置读写位置, 这五个是必须的, 那经过几十年的演进是否存在第六个更有用的函数呢?
...
> [!NOTE]
> java 淘汰的 cms gc 算法已经可以称作是一种特殊的黑魔法了.
