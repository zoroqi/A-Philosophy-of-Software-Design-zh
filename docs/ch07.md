...
> [!NOTE]
> 我更喜欢把"直通方法"叫做"透传". "什么都不做传递给另一个方法."
...
> [!NOTE]
> 消除了啥? 把 TextDocument 去掉了还是把 TextArea 去掉了.
...
[[decorator_pattern|装饰器器设计模式]](也称为“包装器”)是一种鼓励跨层复制 API 的模式。装饰对象接受现有对象并扩展其功能;它提供一个与底层对象相似或相同的 API，它的方法调用底层对象的方法。在第 4 章的 Java I/O 示例中，BufferedInputStream 类是一个装饰器:给定一个 InputStream 对象，它提供了相同的 API，但是引入了缓冲。例如，当它的 read 方法被调用来读取单个字符时，它会调用底层 InputStream 上的 read 来读取更大的块，并保存额外的字符来满足未来的 read 调用。另一个例子出现在窗口系统中:Window 类实现了一个不能滚动的窗口的简单形式，而 ScrollableWindow 类通过添加水平和垂直滚动条来装饰窗口类。

> [!NOTE]
> ![[decorator_pattern|装饰器器设计模式]]
...
- 最后，问问自己新功能是否真的需要包装现有功能：是否可以将其实现为独立于基类的类？在窗口示例中，滚动条可能与主窗口分开实现，而无需包装其所有现有功能。
...
> [!NOTE]
> 这种情况是最烦的, 最常见的的狗血的处理方案就是 "[[the_garbage_can_objects|垃圾桶对象]]", 通过一个名叫 extra/other 来实现传递.
...
> [!NOTE]
> 实际上 spring 采用的就是类似全局变量的方案, 只是在 spring 体系中叫做 Bean. Bean 默认是单例的, 单例就是一种特殊的全局变量.
...
> [!NOTE]
> context 的肆意使用就是 [[the_garbage_can_objects|垃圾桶对象]] 
...
> [!NOTE]
> context 最害怕的就是随着时间, 所有人开发都依赖这个 context, 为了方便出现通过 context 来传递多个结果和动态修改参数. 并且没有文档, 完全不知道 context 里面有什么.
...
> [!NOTE]
> 真的没有更好的方案了吗? python 的默认参数(修改的时候不会报错), haskell 的 State Monad(类此一种全局变量), golang 选择了 context 方案.
